...
class SteamUserService:
    ...
    async def get_steam_id_from_vanity_url(self, vanity_url: str) -> str | None:
        """
    ...
    async def get_owned_games_by_vanity_url(self, vanity_url: str) -> list[dict]:
        """Fetch owned games for a given vanity URL using Steam API"""
        steam_id = await self.get_steam_id_from_vanity_url(vanity_url)
        if steam_id is None:
            return []
        
        return await self.get_owned_games_by_steam_id(steam_id)

    async def get_owned_games_by_steam_id(self, steam_id: str) -> list[dict]:
