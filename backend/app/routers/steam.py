...
async def get_owned_games_api(steam_id: str, steam_user_service: SteamUserService = Depends(get_steam_user_service), logger = Depends(get_logger)) -> dict[str, list[dict] | str | int]:
    ...
    games = await steam_user_service.get_owned_games_by_steam_id(steam_id)
...
async def get_owned_games_by_vanity_url(vanity_url: str, steam_user_service: SteamUserService = Depends(get_steam_user_service), logger = Depends(get_logger)) -> dict[str, list[dict] | str | int]:
    """Get owned games for a given vanity URL using Steam API"""
    logger.info(f"Fetching owned games for vanity URL: {vanity_url}")
    games = await steam_user_service.get_owned_games_by_vanity_url(vanity_url)
...
async def get_owned_games_enriched_by_id(steam_id: str, steam_user_service: SteamUserService = Depends(get_steam_user_service), game_service: GameService = Depends(get_games_service), logger = Depends(get_logger)) -> dict[str, list[dict] | str | int]:
    ...
    enriched_games = await game_service.get_multiple_game_info(app_ids)

    enriched_dict = {g.appid: g for g in enriched_games if isinstance(g, GameFetched)}

    enriched_count = 0
    for game in games:
        steam_game = enriched_dict.get(game['appid'])
        if steam_game:
...
async def get_owned_games_enriched(vanity_url: str, steam_user_service: SteamUserService = Depends(get_steam_user_service), game_service: GameService = Depends(get_games_service), logger = Depends(get_logger)) -> dict[str, list[dict] | str | int]:
...
async def get_game_info_by_id(app_id: int, game_service: GameService = Depends(get_games_service), logger = Depends(get_logger)) -> GetGameInfoResponse:
    """Get game enhanced game info by id """
    try:
        data = await game_service.get_game_info_by_id_async(app_id)
        if data is None:
            raise Exception
