...
import { showErrorPopup, showSuccessPopup, resetPopupVisible, getPopupVisible } from './ui.js';
...
export function fetchSteamLibrary(currentLang) {
  ...
  if (!inputValue) {
    showErrorPopup(steamInputMode === 'id' ? 'Please enter Steam ID' : 'Please enter Steam nickname', currentLang);
    return;
  }
  ...
  fetch(url)
    ...
    .then(data => {
      ...
      if (data.error || data.status === 404) {
        showErrorPopup(data.message || data.error || 'Failed to fetch library', currentLang);
        return;
      }
